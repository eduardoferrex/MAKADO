...
export default function WhatWeDo() {
  ...
  const pillars: Pillar[] = [
    ...
    },
    {
      ...
      details: [
        "Design unique criado do zero (sem usar modelos prontos).",
  ...
  return (
    ...
    >
      {/* Background decoration with zinc/white ambient light */}
      <div className="absolute top-[20%] left-[-10%] w-[450px] h-[450px] bg-zinc-500/[0.015] rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-[20%] right-[-10%] w-[450px] h-[450px] bg-zinc-500/[0.01] rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="max-w-2xl">
            ...
            <h2 className="text-3xl md:text-5xl font-display font-light leading-tight tracking-tight">
              Como ajudamos sua empresa <span className="font-extrabold bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent">a vender muito mais</span>
            </h2>
          ...
          <div className="flex-shrink-0">
            <MagneticButton 
              ...
              className="px-5 py-3.5 rounded-full border border-white/10 hover:border-white/30 bg-zinc-900/10 text-[11px] text-zinc-300 hover:text-black hover:bg-white font-mono uppercase tracking-wider transition-all duration-300 flex items-center gap-2.5 cursor-pointer group shadow-[0_0_20px_rgba(255,255,255,0.03)]"
            >
          ...
          {pillars.map((pillar, idx) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="p-6 md:p-8 rounded-3xl bg-zinc-950/30 backdrop-blur-md border border-white/[0.05] hover:border-white/30 hover:bg-zinc-950/60 transition-all duration-500 flex flex-col justify-between relative overflow-hidden group min-h-[360px] shadow-[0_10px_30px_rgba(0,0,0,0.1)]"
              >
                {/* Visual white subtle glow on hover */}
                <div className="absolute top-0 right-0 w-32 h-32 bg-white/[0.015] blur-2xl rounded-full translate-x-12 -translate-y-12 group-hover:scale-150 transition-all duration-500" />
                ...
                <div>
                  <div className="flex items-center justify-between mb-6 border-b border-white/[0.03] pb-4">
                    ...
                    <div className="flex gap-1.5">
                      {pillar.features.map((feature, fIdx) => (
                        <span 
                          key={fIdx} 
                          className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-[8px] font-mono text-zinc-300 uppercase tracking-widest"
                        >
                          {feature}
                        </span>
                      ))}
                  ...
                  {/* Title & Description */}
                  <h3 className="text-lg md:text-xl font-display font-light text-white mb-3 group-hover:text-zinc-200 transition-colors">
                    {pillar.title}
                  </h3>
                  ...
                  <ul className="space-y-2">
                    {pillar.details.map((detail, dIdx) => (
                      <li key={dIdx} className="flex items-start gap-2 text-xs text-zinc-500 font-light leading-relaxed">
                        <span className="w-1.5 h-1.5 rounded-full bg-zinc-400/40 mt-1.5 flex-shrink-0" />
                        <span>{detail}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="mt-8 pt-4 border-t border-white/[0.02] flex items-center justify-between">
                  <span className="text-[9px] font-mono uppercase tracking-wider text-zinc-600 group-hover:text-zinc-400 transition-colors">
                  ...
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-8 h-8 rounded-full border border-white/5 bg-white/[0.02] group-hover:bg-white group-hover:text-black flex items-center justify-center text-zinc-400 transition-all duration-300"
                  >
        ...
        <motion.div 
          ...
          viewport={{ once: true, margin: "-100px" }}
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          className="mt-16 md:mt-20 p-6 md:p-10 rounded-3xl bg-zinc-950/40 backdrop-blur-md border border-white/10 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_15px_40px_rgba(0,0,0,0.2)] hover:border-white/20 transition-all duration-300"
        >
          <div className="absolute top-0 left-0 w-32 h-32 bg-white/[0.015] blur-2xl rounded-full" />
          
          <div className="max-w-xl relative z-10 text-center md:text-left">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 mb-3">
              <Flame className="w-3.5 h-3.5 text-zinc-400 animate-pulse" />
              <span className="text-[8px] font-mono text-zinc-300 tracking-widest uppercase">Alta Tecnologia</span>
            </div>
            <h3 className="text-lg md:text-xl font-display font-light text-white leading-snug">
              Chega de sites lentos e travados. Seu site com <span className="font-extrabold text-zinc-200">tecnologia rápida e moderna</span>.
            </h3>
          ...
          <div className="relative z-10 flex-shrink-0 w-full md:w-auto">
            <MagneticButton
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full md:w-auto px-6 py-3.5 rounded-full bg-white text-black font-semibold text-xs hover:bg-zinc-200 transition-all duration-300 flex items-center justify-center gap-2 shadow-[0_10px_20px_rgba(255,255,255,0.02)] cursor-pointer"
            >
