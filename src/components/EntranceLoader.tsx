...
export default function EntranceLoader({ onComplete }: EntranceLoaderProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[40%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-zinc-800/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="text-center space-y-6 max-w-sm w-full relative z-10">
        ...
        >
          <div className="w-14 h-14 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center shadow-xl">
            <OfficialLogo size={32} animate={true} className="text-white" />
          </div>
        </motion.div>
        ...
        <div className="w-full h-[2px] bg-zinc-900 rounded-full overflow-hidden relative border border-zinc-800/50">
          <motion.div
            className="absolute left-0 top-0 bottom-0 bg-gradient-to-r from-zinc-500 to-white"
      ...
      <div className="absolute left-6 bottom-6 flex items-center gap-2 text-[8px] font-mono text-zinc-500 tracking-widest uppercase">
        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
        <span>Vite + React + Tailwind</span>
      </div>
      <div className="absolute right-6 bottom-6 text-[8px] font-mono text-zinc-500 tracking-widest uppercase">
